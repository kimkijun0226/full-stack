...
import { AlertTriangle, ArrowLeft, Camera, CircleUser, Eye, EyeOff, Loader2, ShieldCheck, Trash2 } from "lucide-react";
...
export default function ProfilePage() {
  ...
  /* ── Handlers ── */
  const onVerify = async (values: VerifyForm) => {
    const { error } = await supabase.auth.signInWithPassword({
      email: user.email,
      password: values.password,
    });
    if (error) {
      verifyForm.setError("password", { message: "비밀번호가 올바르지 않습니다." });
      return;
    }
    setIsVerified(true);
  };
    ...
    return (
      <main className="w-full min-h-screen flex justify-center p-4 sm:p-6">
        <div className="w-full max-w-sm flex flex-col gap-6 py-4">
          ...
          <section className="flex flex-col items-center gap-6 rounded-2xl border border-border bg-card p-6">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-foreground/8">
              <ShieldCheck className="h-8 w-8 text-foreground/60" />
            </div>
            <div className="text-center">
              <h3 className="font-semibold text-foreground">본인 인증이 필요합니다</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                프로필을 수정하거나 계정을 관리하려면
                <br />
                비밀번호를 확인해주세요.
              </p>
            </div>

            <form onSubmit={verifyForm.handleSubmit(onVerify)} className="w-full flex flex-col gap-4">
              <Controller
                name="password"
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    ...
                    <div className="relative">
                      <Input type={showVerifyPw ? "text" : "password"} placeholder="현재 비밀번호" {...field} />
                      <EyeBtn show={showVerifyPw} toggle={() => setShowVerifyPw(!showVerifyPw)} />
                    </div>
              ...
              <Button type="submit" disabled={verifyForm.formState.isSubmitting} className="w-full">
                {verifyForm.formState.isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : "확인"}
              </Button>
            </form>
          </section>
        </div>
