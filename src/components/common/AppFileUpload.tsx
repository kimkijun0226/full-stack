...
export function AppFileUpload({ file, setFile }: AppFileUploadProps) {
  ...
  const handleRenderPreview = () => {
    ...
    return (
      <div className="w-full flex items-center justify-center aspect-video bg-card rounded-lg">
        <Button size={"icon"} variant={"ghost"} onClick={() => fileInputRef.current?.click()} className="w-full h-full">
          <Image />
        </Button>
      </div>
    );
